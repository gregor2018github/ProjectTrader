...
The `Doors` object layer holds one Tiled **point per front door**, named after the direction a person faces walking out of it (`down`, `up`, `left`, `right`); the Tiled object *id* identifies the door and is what a townsperson remembers. Each `Door` (`src/models/door.py`) keeps three spots on one line: the *threshold* (the Tiled point itself, inside the wall — nobody ever stands there), the *step*, the first walkable spot that many whole tiles out (a door with none within `MAX_STEP_TILES` is dropped with a warning), and between them the *entry*, felt out pixel by pixel as the deepest spot `TMXMap.is_standable()` still allows. Townsfolk walk entry-to-entry and fade over a fixed `DOORWAY_FADE_TILES`, never onto the point itself — reaching it would put them behind the building, which y-sorts on its own baseline.
